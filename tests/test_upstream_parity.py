"""Upstream self-test, ported against the vendored engine.

This is ``upstream/selftest.py`` with its loader preamble replaced and nothing
else touched: the assertion bodies below are upstream's, verbatim and in
upstream order. Upstream loads its sibling modules from the working directory
and rewrites ``brain.py``'s relative imports with string replacement; the port
has the same modules as a real package, so it simply imports them.

Upstream's ``check(name, cond)`` kept a pass/fail tally and printed a summary.
Here it records the same results into ``RESULTS`` and pytest reports one test
per check, so a failure names the exact interaction that broke.

Do not rewrite these assertions. They are the port's parity evidence: each one
pins a law interaction that upstream found the hard way.
"""
...
UPSTREAM_DIR = pathlib.Path(brain.__file__).parent

BS = brain.build_system
NEG = brain.build_negative

# Assertions that actually fire, not `check(` calls in the source: several
# sections sweep every accent, style or music key in a loop, so 418 call sites
# execute 433 times. Deterministic — the loops iterate fixed engine constants.
EXPECTED_ASSERTIONS = 433
...
from prompt_master.prompt_engine.upstream import wardrobe as W
...
# seed variety
from prompt_master.prompt_engine.upstream import wardrobe as W2
picks = set()
for sd in range(12):
...
from prompt_master.prompt_engine.upstream import identity as _ID
...
from prompt_master.prompt_engine.upstream import identity as _I2
...
from prompt_master.prompt_engine.upstream import shotscript as _SS
w = _SS._windows(16, 5)
...
from prompt_master.prompt_engine.upstream import styles as _ST
...
from prompt_master.prompt_engine.upstream import negative as NG
...
from prompt_master.prompt_engine.upstream import accents as AC
...
_rs = (UPSTREAM_DIR / "routes.py").read_text(encoding="utf-8")
_ns = {}
...
from prompt_master.prompt_engine.upstream import music as MU
...
from prompt_master.prompt_engine.upstream import wardrobe as WD
_hits = []
...
# ── pytest reporting ─────────────────────────────────────────────────────────
# The checks above run once at import. Each becomes its own test so a failure
# names the interaction rather than a line number.
...
@pytest.mark.parametrize("name,ok", RESULTS, ids=_ids())
def test_upstream_interaction(name, ok):
    assert ok, f"upstream self-test assertion failed: {name}"


def test_every_assertion_ran():
    """Guards against the script aborting early and silently shrinking coverage."""
    assert len(RESULTS) == EXPECTED_ASSERTIONS, (
        f"expected {EXPECTED_ASSERTIONS} upstream assertions, collected "
        f"{len(RESULTS)} — the self-test aborted partway"
    )
